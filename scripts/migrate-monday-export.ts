...
import { PrismaClient, Stage, ServiceType, Source } from "@prisma/client";
...
async function parseWorkbook(path: string): Promise<ParsedRecord[]> {
...
function printSummary(records: ParsedRecord[]) {
  const byStage = new Map<string, number>();
  for (const r of records) byStage.set(r.stage, (byStage.get(r.stage) ?? 0) + 1);
  ...
  console.log(`Unique contacts (by email/phone/name): ${uniqueContacts}`);
}
...
async function run() {
  ...
  const records = await parseWorkbook(path);
  ...
  console.log(`\nImported ${created} deals across ${contactIdByKey.size} contacts.`);
